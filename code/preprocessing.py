...
def set_data(start_time, end_time, time_window, emotion):
    ...
    for STFW in range(start_time, end_time, time_window):  # STFW: start time for window
        ...
        if check_noise(STFW, latest_time):
            continue
        ...
        breathing_avg.append(breathing_df[column_name['breathing']][idx:idx + hz['breathing'] * time_window].sum() / (
                time_window * hz['breathing']))
        breathing_med.append(
            statistics.median(breathing_df[column_name['breathing']][idx:idx + hz['breathing'] * time_window]))
...
output_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'input.csv')
...
min_recorded_time_in_sec = min(len(hr_df) / hz['hr'], len(eda_df) / hz['eda'], len(ibi_df) / hz['ibi'],
                               len(breathing_df) / hz['breathing'], len(ecg_df) / hz['ecg'])
...
data = {'labeling': labeling,
        ...
        'breathing_max': breathing_max, 'breathing_min': breathing_min, 'breathing_avg': breathing_avg,
        'breathing_med': breathing_med, 'breathing_std': breathing_std,
