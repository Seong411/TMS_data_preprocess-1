...
def get_start_RR_idx(start_time, zephyr_time, cnt):
	...
	while True:
		...
		if start_idx>=cnt:
			break
		sum_column = sum_column + rr_df[column_name['RtoR']][start_idx]/1000
...
def set_data(start_time, end_time, zephyr_time, time_window, emotion):
	...
	for STFW in range(start_time, end_time, time_window): # STFW: start time for window
		...
		for STFW__ in range(STFW - int(zephyr_time), STFW + time_window - int(zephyr_time), hrv_interval):
			start_idx = get_start_RR_idx(STFW__ - hrv_window, zephyr_time, len(rr_df) - 1)
			end_idx = get_end_RR_idx(STFW__ - hrv_window, hrv_window, start_idx, 0, len(rr_df) - 1)
			# 실제 rr_diff_square는 한칸씩 밀려있는 거 고려 해줘야 함
			hrv.append(math.sqrt(sum(rr_diff_square[start_idx:end_idx]) / (end_idx - start_idx + 1)))


		n = len(hrv)  # Length of signal
		NFFT = n  # ?? NFFT=2^nextpow2(length(y))  ??

		Y = numpy.fft.fft(hrv) / NFFT  # fft computing and normaliation
		Y = Y[range(math.trunc(NFFT / 2))]  # single sied frequency range
		amplitude_Hz = 4 * abs(Y) ** 2

		LF.append(sum(amplitude_Hz[int(0.04 * 240 + 0.5):int(0.15 * 240 + 0.5)]))
		HF.append(sum(amplitude_Hz[int(0.14 * 240 + 0.5):int(0.4 * 240 + 0.5)]))
		#LF_HF_ratio.append(sum(amplitude_Hz[int(0.04 * 240 + 0.5):int(0.15 * 240 + 0.5)])/float(sum(amplitude_Hz[int(0.14 * 240 + 0.5):int(0.4 * 240 + 0.5)])))
		'''
...
column_name['bvp'] = list(bvp_df)[0]

'''
for i in range(1, len(rr_df), 1):
    if rr_df[column_name['RtoR']][i] > 1400 or rr_df[column_name['RtoR']][i] < 450:
        rr_df[column_name['RtoR']][i] = (rr_df[column_name['RtoR']][i - 1] + rr_df[column_name['RtoR']][i + 1]) / 2

for i in range(len(rr_df) - 1):
    rr_diff_square.append((rr_df[column_name['RtoR']][i + 1] - rr_df[column_name['RtoR']][i]) ** 2)
'''
...
latest_time = max(start_time['hr'], start_time['eda'], start_time['ibi'], start_time['bvp'])
...
min_recorded_time_in_sec = min(len(hr_df)/hz['hr'], len(eda_df)/hz['eda'], len(ibi_df)/hz['ibi'], len(bvp_df)/hz['bvp'])
...
data = {'labeling': labeling,
		...
		'eda_max': eda_max, 'eda_min': eda_min, 'eda_avg': eda_avg, 'eda_med': eda_med, 'eda_std': eda_std, 'sbp': sbp}
